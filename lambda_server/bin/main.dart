...
void main(List<String> arguments) async {
  final moviesProvider = MovieInfoProvider(
    RutorTrackerDataSource(DioHtmlPageProvider(), Platform.environment['RUTOR_ADDRESS']),
  ...
  final handler = (Context context, AwsCloudwatchEvent event) async {
    ...
    await ds.setImageBasePath(await moviesProvider.getImageBasePath());
    await ds.setMovieInfo(await moviesProvider.getTopSeedersFhdMovies());
    return InvocationResult(context.requestId, '');
  };
  Runtime()
    ..registerHandler<AwsCloudwatchEvent>(Platform.environment['AWS_LAMBDA_HANDLER_NAME'], handler)
    ..invoke();
}
